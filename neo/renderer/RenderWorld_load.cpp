...
// HUMANHEAD pdm: Support for level appending
	...
	if ( bAppending ) {
		common->Printf("Appending deathwalk proc: %s of %s\n", dwMapName, name );
	} else {
		common->Printf( "No deathwalk map in %s\n", name );
	}
#endif
...
// HUMANHEAD pdm: Support for level appending
	...
	while ( bAppending ) {
		filename = dwMapName;
		filename.SetFileExtension(PROC_FILE_EXT);
		common->Printf( "Appending deathwalk proc: %s\n", filename.c_str() );
		src = new idLexer(filename, LEXFL_NOSTRINGCONCAT | LEXFL_NODOLLARPRECOMPILE);

		if ( !src->IsLoaded() ) {
			numAppendPortalAreas = 0;
			common->Warning( "Appending deathwalk proc -> idRenderWorldLocal::InitFromMap: %s not found\n", filename.c_str() );
			break;
		}
		...
		if ( !src->ReadToken( &token ) || token.Icmp( PROC_FILE_ID ) ) {
			numAppendPortalAreas = 0;
			common->Warning( "Appending deathwalk proc -> idRenderWorldLocal::InitFromMap: bad id '%s' instead of '%s'\n", token.c_str(), PROC_FILE_ID );
			break;
		}
		...
		while ( 1 ) {
			...
			if ( token == "model" ) {
				...
				if( !idStr::Icmpn( tmp, "_area", 5 ) ) {
					common->Printf( "Appending deathwalk proc: parse area(%s) to portal area(%d)\n", tmp.c_str(), numPortalAreas - 1 );
		...
		common->Printf( "Appending deathwalk proc: find area model(%d %s)\n", numPortalAreas - 1, model ? model->Name() : "null" );
		if( model ) {
			int numAreaNodes_orig = numAreaNodes;
			numAreaNodes += 6;
			common->Printf( "Appending deathwalk proc: push 6 nodes to list front(%d -> %d)\n", numAreaNodes_orig, numAreaNodes );

			areaNode_t *areaNodes_orig = areaNodes;
			areaNodes = (areaNode_t *)R_ClearedStaticAlloc( numAreaNodes * sizeof( areaNodes[0] ) );

			common->Printf( "Appending deathwalk proc: copying area nodes(%d)\n", numAreaNodes_orig );
			...
			common->Printf( "Appending deathwalk proc: area bounds(%s, %s)\n", bounds[0].ToString(), bounds[1].ToString() );
		...
		common->Printf( "Appending deathwalk proc finish\n" );
		break;
	}
	delete src;
#endif
