...
const char* GetKeyName( int keyNum, bool localized = true )
{
	...
	// handle scancodes separately, because ImGui uses UTF-8, while the engine uses ISO8859-1
...
struct BindingEntry {
	...
	BindingEntrySelectionState HandleBindPopup( const char* popupName, bool newOpen )
	{
		...
		if ( ImGui::BeginPopupModal( popupName, NULL, ImGuiWindowFlags_AlwaysAutoResize ) )
		{
			...
			// Note: gamepad Start also generates K_ESCAPE in the engine
...
static void DrawBindingsMenu()
{
	...
	const float commandColumnWidth = ImGui::CalcTextSize( "Advance Settings Menu" ).x + defFramePadding.x * 2.0f;
...
static CVarOption videoOptionsImmediately[] = {
	...
	CVarOption( "r_windowResizable", "Make the window resizable", OT_BOOL ),
...
static void SetVideoStuffFromCVars()
{
	...
	qualityPreset = com_imageQuality.GetInteger();
	if ( qualityPreset == -1 ) {
...
static void DrawAudioOptionsMenu()
{
	...
	ImGui::SeparatorText( "Settings that require restarting the game" );
...
void Com_DrawDhewm3SettingsMenu()
{
	...
	ImGui::Begin("Advance Settings Menu", &showSettingsWindow);
