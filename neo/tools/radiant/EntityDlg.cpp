...
void CEntityDlg::OnSize( UINT nType, int cx, int cy )
{
	...
	btnStopAnim.SetWindowPos( NULL, rightAnimAreaBorder - crect.Width(), top + 8, crect.Width(), crect.Height(), SWP_SHOWWINDOW );
	...
	staticFrame.SetWindowPos( NULL, rightAnimAreaBorder - crect.Width(), top + ( 34 * scaling_factor ), crect.Width(), crect.Height(), SWP_SHOWWINDOW );

	left = rightAnimAreaBorder - crect.Width() - s4;

	slFrameSlider.GetWindowRect( crect );
	slFrameSlider.SetWindowPos( NULL, left - crect.Width(), top + ( 32 * scaling_factor ), crect.Width(), crect.Height(), SWP_SHOWWINDOW );
...
static int TabOrder[] = { IDC_COMBO_CLASS,
	IDC_BUTTON_CREATE,
	// IDC_EDIT_INFO,
	IDC_LIST_KEYVAL,
	IDC_EDIT_KEY,
