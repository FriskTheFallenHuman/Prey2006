...
void   CXYWnd::OnMouseMove( UINT nFlags, CPoint point )
{
	...
	if( g_PrefsDlg.m_bChaseMouse == TRUE && ( point.x < 0 || point.y < 0 || point.x > m_nWidth || point.y > m_nHeight ) && GetCapture() == this )
	{
	...
	if( !m_bRButtonDown )
	{
		...
		if( PointMode() )
		{
			...
			else
			{
				...
				{
					if( idMath::Diff( g_PointPoints[n].m_ptClip[nDim1], tdp[nDim1] ) < 3 && idMath::Diff( g_PointPoints[n].m_ptClip[nDim2], tdp[nDim2] ) < 3 )
					{
						bCrossHair	   = true;
		...
		else if( ClipMode() )
		{
			...
			else
			{
				...
				if( g_Clip1.Set() )
				{
					if( idMath::Diff( g_Clip1.m_ptClip[nDim1], tdp[nDim1] ) < 3 && idMath::Diff( g_Clip1.m_ptClip[nDim2], tdp[nDim2] ) < 3 )
					{
						bCrossHair	  = true;
				...
				if( g_Clip2.Set() )
				{
					if( idMath::Diff( g_Clip2.m_ptClip[nDim1], tdp[nDim1] ) < 3 && idMath::Diff( g_Clip2.m_ptClip[nDim2], tdp[nDim2] ) < 3 )
					{
						bCrossHair	  = true;
				...
				if( g_Clip3.Set() )
				{
					if( idMath::Diff( g_Clip3.m_ptClip[nDim1], tdp[nDim1] ) < 3 && idMath::Diff( g_Clip3.m_ptClip[nDim2], tdp[nDim2] ) < 3 )
					{
						bCrossHair	  = true;
		...
		else if( PathMode() )
		{
			...
			else
			{
				...
				{
					if( idMath::Diff( g_PathPoints[n].m_ptClip[nDim1], tdp[nDim1] ) < 3 && idMath::Diff( g_PathPoints[n].m_ptClip[nDim2], tdp[nDim2] ) < 3 )
					{
						bCrossHair	  = true;
...
void CreateSmartEntity( CXYWnd* pWnd, int x, int y, const char* pName )
{
	...
	{
		ShowInfoDialog( "Select the path of the train by left clicking in XY, YZ and/or XZ views. You can move an already dropped point by grabbing and moving it. When you are finished, press ENTER "
						"to accept and create the entity and path(s), press ESC to abandon the creation" );
		g_bPathMode	 = true;
		g_nPathLimit = 0;
		g_nPathCount = 0;
		g_bSmartGo	 = true;
...
void CXYWnd::OnEntityCreate( unsigned int nID )
{
	if( m_mnuDrop.GetSafeHmenu() )
	{
		...
		if( strItem.CompareNoCase( "Add to..." ) == 0 || strItem.CompareNoCase( "Remove" ) == 0 || strItem.CompareNoCase( "Name..." ) == 0 || strItem.CompareNoCase( "New group..." ) == 0 )
		{
...
bool MergeMenu( CMenu* pMenuDestination, const CMenu* pMenuAdd, bool bTopLevel /*=false*/ )
{
	...
	for( int iLoop = 0; iLoop < iMenuAddItemCount; iLoop++ )
	{
		...
		else
		{
			...
			if( pMenuDestination->InsertMenu( iInsertPosDefault, MF_BYPOSITION | MF_POPUP | MF_ENABLED, ( UINT_PTR )hNewMenu, sMenuAddString ) )
			{
...
void CXYWnd::HandleDrop()
{
	if( !m_mnuDrop.GetSafeHmenu() ) // first time, load it up
	{
		...
		for( e = eclass; e; e = e->next )
		{
			...
			if( n_ > 0 )
			{
				...
				else
				{
					if( pChild )
					{
						pMakeEntityPop->AppendMenu( MF_POPUP, reinterpret_cast<UINT_PTR>( pChild->GetSafeHmenu() ), strActive );
		...
		{
			m_mnuDrop.AppendMenu( MF_POPUP, reinterpret_cast<UINT_PTR>( pMakeEntityPop->GetSafeHmenu() ), "Make Entity" );
		}
	}
...
void   CXYWnd::XY_MouseDown( int x, int y, int buttons )
{
	...
	if( ( g_PrefsDlg.m_nMouseButtons == 3 && m_nButtonstate == MK_MBUTTON ) || ( g_PrefsDlg.m_nMouseButtons == 2 && m_nButtonstate == ( MK_SHIFT | MK_CONTROL | MK_RBUTTON ) ) )
	{
...
void CXYWnd::Paste()
{
	...
	if( g_brClipboard.next != &g_brClipboard || g_enClipboard.next != &g_enClipboard )
	{
		...
		for( idEditorEntity* pEntity = g_enClipboard.next; pEntity != NULL && pEntity != &g_enClipboard; pEntity = pEntity->next )
		{
