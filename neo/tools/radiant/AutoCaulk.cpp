...
void Select_AutoCaulk()
{
	...
	for( idEditorBrush* pSelectedBrush = selected_brushes.next; pSelectedBrush != &selected_brushes; pSelectedBrush = next )
	{
		...
		{
			idEditorBrush* snext;
			for( idEditorBrush* pScannedBrush = ( iBrushListToScan ? active_brushes.next : selected_brushes.next ); pScannedBrush != ( iBrushListToScan ? &active_brushes : &selected_brushes );
				 pScannedBrush				  = snext )
			{
						...
						*/
						{
							...
							if( v3ZeroTest == v3Zero )
							{
								...
								{
									fTotalDist += fabs( DotProduct( pSelectedFace->plane.Normal(), ( *pSelectedWinding )[0] ) - DotProduct( pSelectedFace->plane.Normal(), ( *pScannedWinding )[i] ) );
								}
