...
static void WriteRenderBump( renderBump_t *rb, int outLinePixels ) {
	...
	R_WriteImage( TYPE_TGA, filename, rb->localPic, 4, width, height, false, "fs_basepath" );
	...
	if ( rb->saveGlobalMap ) {
		...
		common->Printf( "writing %s (%i,%i)\n", filename.c_str(), width, height );
		R_WriteImage( TYPE_TGA, filename, rb->globalPic, 4, width, height, false, "fs_basepath" );
	}
	...
	if ( rb->saveColorMap ) {
		...
		common->Printf( "writing %s (%i,%i)\n", filename.c_str(), width, height );
		R_WriteImage( TYPE_TGA, filename, rb->colorPic, 4, width, height, false, "fs_basepath" );
	}
...
//flat = true;
	...
	R_WriteImage( TYPE_TGA, filename, buffer, 4, width, height );
