...
void codec::Segment( int* alist, float* flist, int numElements, float rmse )
{
	...
	if( doopen )
	{
		...
		for( onf = 0; onf < 256; onf++ )
		{
			...
			for( x = 0; x < 4; x++ )
			{
				fy = RMULT * ( float )( codebook2[onf][numc + 0] ) + GMULT * ( float )( codebook2[onf][numc + 1] ) + BMULT * ( float )( codebook2[onf][numc + 2] ) + 0.5f;
				if( fy < 0 )
				{
					fy = 0;
				}
...
void codec::FvqData( byte* bitmap, int size, int realx, int realy, quadcel* pquad, bool clamp )
{
	...
	if( ( ( realx - xxMean ) + searchX ) < 0 || ( ( ( realx - xxMean ) - searchX ) + depthx + size ) > xLen || ( ( realy - yyMean ) + searchY ) < 0 ||
		( ( ( realy - yyMean ) - searchY ) + depthy + size ) > yLen )
	{
		pquad->snr[FCC] = 9999;
		return;
	}
