...
bool rvGENavigator::Create( HWND parent, bool visible )
{
	...
	if( !gApp.GetOptions().GetWindowPlacement( "navigator", mWnd ) )
	{
		...
		SetWindowPos( mWnd, NULL, rParent.right - 10 - ( rNav.right - rNav.left ), rParent.bottom - 10 - ( rNav.bottom - rNav.top ), 0, 0, SWP_NOZORDER | SWP_NOSIZE );
	}
...
LRESULT CALLBACK rvGENavigator::WndProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
	...
	switch( msg )
	{
		...
		case WM_DRAWITEM:
		{
			...
			if( window )
			{
				...
				if( wrapper->GetVariableDict().GetNumKeyVals() || wrapper->GetScriptDict().GetNumKeyVals() )
				{
					DrawIconEx( dis->hDC,
						dis->rcItem.right - 16,
						( dis->rcItem.bottom + dis->rcItem.top ) / 2 - 6,
						( dis->itemState & ODS_SELECTED ) ? nav->mScriptsLightIcon : nav->mScriptsIcon,
						13,
						13,
						0,
						NULL,
		...
		case WM_CREATE:
		{
			...
			nav->mTree = CreateWindowEx( 0,
				"SysListView32",
				"",
				WS_VSCROLL | WS_CHILD | WS_VISIBLE | LVS_REPORT | LVS_OWNERDRAWFIXED | LVS_NOCOLUMNHEADER | LVS_SHOWSELALWAYS,
				0,
				0,
				0,
				0,
				hWnd,
				( HMENU )IDC_GUIED_WINDOWTREE,
				win32.hInstance,
				0 );
