...
public:
	...
	virtual ~StageViewItem() { label.Clear(); }

	ID_INLINE void	 SetLabel( const char* _label ) { label = _label; }
	ID_INLINE idStr& GetLabel() { return label; }

	ID_INLINE int	 GetData() const { return data; }
	ID_INLINE void	 SetData( int _data ) { data = _data; }
