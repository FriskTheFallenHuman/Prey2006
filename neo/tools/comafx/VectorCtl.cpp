...
{
	RotateByXandY( double( -iyMove ) / m_dSensitivity, double( ixMove ) / m_dSensitivity );
}
...
void CVectorCtl::BuildImage( LPDRAWITEMSTRUCT lpDrawItemStruct )
{
	...
	for( int x = 0; x < m_iWidth; x++ )		 // Scan all columns
		for( int y = 0; y < m_iHeight; y++ ) // Scan all rows
		{
			...
			{
				double vx = double( xf ) / double( m_iRadius ), vy = double( yf ) / double( m_iRadius ),
...
{
	double NL = dx * m_dVec[0] + dy * m_dVec[1] + dz * m_dVec[2], RV = 2.0 * NL,
		   // rx = m_dVec[0] - (dx * RV),
		   // ry = m_dVec[1] - (dy * RV),
		rz = m_dVec[2] - ( dz * RV );
...
void CVectorCtl::RotateByXandY( double XRot, double YRot )
{
	...
	double cx = cos( XRot ), sx = sin( XRot ), cy = cos( YRot ), sy = sin( YRot ), dx = m_dVec[0] * cy + m_dVec[1] * sx * sy + m_dVec[2] * cx * sy, dy = m_dVec[1] * cx - m_dVec[2] * sx,
...
void CVectorCtl::CreateBackground()
{
	...
	{
		double r = GetRValue( m_clrBackgroundStart ), g = GetGValue( m_clrBackgroundStart ), b = GetBValue( m_clrBackgroundStart ),
			   rd = double( GetRValue( m_clrBackgroundEnd ) - r ) / double( m_iHeight ), gd = double( GetGValue( m_clrBackgroundEnd ) - g ) / double( m_iHeight ),
	...
	int iTmpWidth = tmpBitmap.bmWidth, iTmpHeight = tmpBitmap.bmHeight;
